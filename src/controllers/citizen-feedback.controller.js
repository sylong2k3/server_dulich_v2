...
class CitizenFeedbackController {
  ...
  static getById = asyncHandler(async (req, res) => OK(res, 'Chi tiết phản ánh', await CitizenFeedbackService.getById(parseInt(req.params.id), {
    userId: req.user?.id,
  ...
  static update = asyncHandler(async (req, res) => {
    ...
    const result = await CitizenFeedbackService.update(parseInt(req.params.id), req.body, req.user.id, isAdmin);
    return OK(res, 'Cập nhật phản ánh thành công', result);
  });

  static updateStatus = asyncHandler(async (req, res) => {
    const result = await CitizenFeedbackService.updateStatus(parseInt(req.params.id), req.body);
    return OK(res, 'Cập nhật trạng thái thành công', result);
  });

  static updateModerationStatus = asyncHandler(async (req, res) => {
    const result = await CitizenFeedbackService.updateModerationStatus(parseInt(req.params.id), req.body);
    return OK(res, 'Cập nhật kiểm duyệt thành công', result);
  });

  static delete = asyncHandler(async (req, res) => {
    await CitizenFeedbackService.delete(parseInt(req.params.id));
