...
class FestivalController {
    static getAll = asyncHandler(async (req, res) => OK(res, 'Danh sách lễ hội', await FestivalService.getAll(req.query, { user: req.user })));
    static getCalendar = asyncHandler(async (req, res) => OK(res, 'Lịch lễ hội', { events: await FestivalService.getCalendar(req.query) }));
    static getById = asyncHandler(async (req, res) => OK(res, 'Chi tiết lễ hội', await FestivalService.getById(req.params.id, { user: req.user })));
